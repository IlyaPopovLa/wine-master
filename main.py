...
def load_wine_data(excel_file_path):
    wine_table = pandas.read_excel(excel_file_path, sheet_name='Лист1')
    ...
    return wines_category


def render_site(wines_category, delta_year, year_form):
    env = Environment(
...
def parse_args():
    parser = argparse.ArgumentParser(description='Wine shop site generator')
    parser.add_argument('--data-path', default=os.getenv('WINE_DATA_PATH', 'data_wine.xlsx'),
    help='Path to Excel data file (default: data_wine.xlsx or .env WINE_DATA_PATH)')
...
def main():
    ...
    wines_category = load_wine_data(data_path)
